# Insessorial Lifestyle summary statsitics
...
######## INSESSORIAL
insessorial <- readRDS("Data/species_matrix/insessorial_df.RDS")

insessorial <- insessorial %>%
  filter(OBSERVATION_COUNT >= 1) 

# Calculating total richness, total checklists, total observers, total abundance
s_matrix <- insessorial %>%
...
mean_diversity_across_checklists <- insessorial %>%
...
resampled_richness_per_locality <- bind_rows(lapply(unique(insessorial$LOCALITY_ID), function(x) downsample_function(x, 50)))
...
saveRDS(s_matrix, file = "Data/species_matrix/insessorial_matrix.RDS")
