# aerial Lifestyle summary statsitics
...
######## INSESSORIAL
aerial  <- readRDS("Data/species_matrix/aerial_df.RDS")
...
mean_diversity_across_checklists <- aerial  %>%
...
resampled_richness_per_locality <- bind_rows(lapply(unique(aerial$LOCALITY_ID), function(x) downsample_function(x, 50)))
...
saveRDS(s_matrix, file = "Data/species_matrix/aerial_matrix.RDS")
