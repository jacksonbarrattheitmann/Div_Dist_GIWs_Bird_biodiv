# generalist  Lifestyle summary statsitics
...
######## INSESSORIAL
generalist  <- readRDS("Data/species_matrix/generalist_df.RDS")
...
mean_diversity_across_checklists <- generalist  %>%
...
resampled_richness_per_locality <- bind_rows(lapply(unique(generalist$LOCALITY_ID), function(x) downsample_function(x, 50)))
...
saveRDS(s_matrix, file = "Data/species_matrix/generalist_matrix.RDS")
