# Aquatic Lifestyle summary statsitics
...
######## Aquatic
aquatic <- readRDS("Data/species_matrix/aquatic_df.RDS")
...
test <- aquatic %>%
  filter(LOCALITY_ID == "L442904")

length(unique(test$SAMPLING_EVENT_IDENTIFIER))
...
mean_diversity_across_checklists <- aquatic %>%
...
resampled_richness_per_locality <- bind_rows(lapply(unique(aquatic$LOCALITY_ID), function(x) downsample_function(x, 14)))
...
saveRDS(s_matrix, file = "Data/species_matrix/aquatic_matrix.RDS")
